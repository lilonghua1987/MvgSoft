...
namespace libviso
{
	...
	void VisualOdometryStereo::getPoints(std::vector<Matrix>& points)
	{
		std::cout << "getPoints" << std::endl;
		points = std::vector<Matrix>(p_matched.size());
		for (size_t i = 0; i < p_matched.size(); i++)
		{
			...
			point.val[0][3] = p_matched[i].u1p;
			point.val[0][4] = p_matched[i].v1p;
	...
	{
		std::cout << "estimateMotion" << std::endl;
		// return value
		bool success = true;

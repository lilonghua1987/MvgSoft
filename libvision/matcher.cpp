...
namespace libviso
{
	...
	void Matcher::pushBack(uint8_t *I1, uint8_t* I2, int32_t* dims, const bool replace) {
		...
		std::cout << "compute new features for current frame" << std::endl;
	...
	void Matcher::matchFeatures(int32_t method, Matrix *Tr_delta) {
		...
		{
			//std::cout << "stereo match" << std::endl;
	...
	{
		std::cout << "bucketFeatures" << std::endl;
		// find max values
		float u_max = 0;
		float v_max = 0;
	...
	void Matcher::computeFeatures(uint8_t *I, const int32_t* dims, int32_t* &max1, int32_t &num1, int32_t* &max2, int32_t &num2, uint8_t* &I_du, uint8_t* &I_dv, uint8_t* &I_du_full, uint8_t* &I_dv_full) {
		...
		if (!param.half_resolution) {
			...
			VFilter::sobel5x5(I, I_du, I_dv, dims[2], dims[1]);
			VFilter::blob5x5(I, I_f1, dims[2], dims[1]);
			VFilter::checkerboard5x5(I, I_f2, dims[2], dims[1]);
		}
		else {
			...
			VFilter::sobel5x5(I_matching, I_du, I_dv, dims_matching[2], dims_matching[1]);
			VFilter::sobel5x5(I, I_du_full, I_dv_full, dims[2], dims[1]);
			VFilter::blob5x5(I_matching, I_f1, dims_matching[2], dims_matching[1]);
			VFilter::checkerboard5x5(I_matching, I_f2, dims_matching[2], dims_matching[1]);
			_mm_free(I_matching);
		}

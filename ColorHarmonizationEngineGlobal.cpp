...
namespace openMVG{
	...
	ColorHarmonizationEngineGlobal::ColorHarmonizationEngineGlobal(const std::string & sImagePath, const std::string & sMatchesPath,
		const std::string sMatchesFile, const std::string & sOutDirectory, const int selectionMethod, const int imgRef)
		: ReconstructionEngine(sImagePath, sMatchesPath, sOutDirectory), sMatchesFile(sMatchesFile), selectionMethod(selectionMethod),
		imgRef(imgRef)
	{
		std::string logPath = "log/log_sfm_ColorHarmonizationEngineGlobal.log";
		...
		std::cout << sImagePath << "\t" << sMatchesPath << "\t" << sMatchesFile << "\t" << sOutDirectory << std::endl;
	}
	...
	bool ColorHarmonizationEngineGlobal::setRefParmas(const int selectionMethod, const int imgRef){
		//Choose image reference
		if (imgRef >= _vec_fileNames.size() || imgRef < 0 || selectionMethod < 0 || selectionMethod > 2)
		{
			std::cout << "The reference image :" << imgRef << "The method :" << selectionMethod << std::endl;
			return false;
		}
		else
		{
			this->imgRef = imgRef;
			std::cout << "Choose your reference image:\t" << imgRef << std::endl;
			this->selectionMethod = selectionMethod;
			std::cout << "Choose your selection method:\t" << selectionMethod << std::endl
				<< "- FullFrame: 0\n"
				<< "- Matched Points: 1\n"
				<< "- VLD Segment: 2\n";
			return true;
	...
	bool ColorHarmonizationEngineGlobal::Process()
	{
		...
		for (std::map< std::pair<size_t, size_t>, std::vector<IndMatch> >::iterator iter = _map_Matches.begin();
  ...
  if (imgRef >= _vec_fileNames.size() || imgRef < 0 || selectionMethod < 0 || selectionMethod > 2)
  {
	  if (!setRefParmas(1, _vec_fileNames.size() / 2 + 1))
	  {
		  return false;
	  }
  }
  ...
  {
	  std::map< std::pair<size_t, size_t>, std::vector<IndMatch> >::const_iterator iter = _map_Matches.begin();
	  std::advance(iter, i);
  ...
  vec_indexToFix.push_back(map_cameraNodeToCameraIndex[imgRef]);
...
#pragma omp parallel for
	  ...
	  std::string out_folder = stlplus::create_filespec(_sOutDirectory,
		  vec_selectionMethod[selectionMethod] + "_" + vec_harmonizeMethod[harmonizeMethod]);
  ...
  return true;
	...
	bool ColorHarmonizationEngineGlobal::ReadInputData()
	{
		...
		// a. Read images names
		{
			  ...
			  iter_imageName++)
		  {
			  ...
			  _vec_imageSize.push_back(std::make_pair(vec_intrinsicGroups[iter_imageName->m_intrinsicId].m_w,
				  vec_intrinsicGroups[iter_imageName->m_intrinsicId].m_h));
		  }
	  }

	  // b. Read matches
	  if (!matching::PairedIndMatchImport(sMatchesFile, _map_Matches))
	  {
	...
	bool ColorHarmonizationEngineGlobal::CleanGraph()
	{
		...
		imageGraph::exportToGraphvizData(stlplus::create_filespec(_sOutDirectory, "cleanedGraph"),putativeGraph.g);

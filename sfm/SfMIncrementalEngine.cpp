...
namespace openMVG{
		...
		_bUseBundleAdjustment(true)
	{
		...
		if (_bHtmlReport)
		{
			_htmlDocStream = unique_ptr<htmlDocument::htmlDocumentStream>(
		...
		std::string logPath = "log/log_sfm_IncrementalReconstructionEngine.log";
	...
	bool IncrementalReconstructionEngine::Process()
	{
		...
		if (!ReadInputData())
		{
			std::cout << "ERROE: ReadInputData" << std::endl;
			return false;
		}
		...
		//-------------------
		if (!InitialPairChoice())
		{
			std::cout << "ERROR : InitialPairChoice " << "initialPairIndex: [" << _initialpair.first << "," << _initialpair.second << "]" << std::endl;
			return false;
		}


		// Initial pair Essential Matrix and [R|t] estimation.
		if (!MakeInitialPair3D())
		{
			std::cout << "ERROE: MakeInitialPair3D" << std::endl;
			return false;
		}
		...
		std::vector<size_t> possibleResectionIndexes;
		while (FindImagesWithPossibleResection(possibleResectionIndexes))
		{
			if (Resection(possibleResectionIndexes))
			{
	...
	bool IncrementalReconstructionEngine::ReadInputData()
	{
...

		{
			...
			std::cout << std::endl << "Track stats" << std::endl;
			{
				...
				for (const auto& trackL : map_Occurence_TrackLength)
				{
					osTrack << "\t" << trackL.first << "\t" << trackL.second << "\n";
				}
				osTrack << "\n";
	...
	bool IncrementalReconstructionEngine::InitialPairChoice()
	{
		if (_initialpair == std::make_pair<size_t, size_t>(0, 0))
		{
			std::cout << std::endl
				...
				<< " Choose the first pair manually by sort order default !\n"
				<< "---------------------------------------------------\n"
				<< std::endl;

			// Display to the user the 10 top Fundamental matches pair
			std::vector< size_t > vec_NbMatchesPerPair;
			for (const auto& match : _map_Matches_F)
			{
				vec_NbMatchesPerPair.push_back(match.second.size());
			}
			// sort in descending order
			using namespace indexed_sort;
			std::vector< sort_index_packet_descend< double, int> > packet(vec_NbMatchesPerPair.size());
			//sort_index_helper(packet_vec, &vec_NbMatchesPerPair[0], std::min((size_t)10, _map_Matches_F.size()));
			sort_index_helper(packet, &vec_NbMatchesPerPair[0], std::min((size_t)1, _map_Matches_F.size()));

			//select the most matches pair
			openMVG::tracks::STLPairWiseMatches::const_iterator iter = _map_Matches_F.begin();
			std::advance(iter, packet[0].index);
			_initialpair.first = iter->first.first;
			_initialpair.second = iter->first.second;
		}
		...
		if (_map_feats.find(_initialpair.first) == _map_feats.end() ||
			_map_feats.find(_initialpair.second) == _map_feats.end())
		{
	...
	bool IncrementalReconstructionEngine::MakeInitialPair3D()
	{
		// Compute robust Essential matrix for ImageId [I,J]
		// use min max to have I < J
		const size_t I = min(_initialpair.first, _initialpair.second);
		const size_t J = max(_initialpair.first, _initialpair.second);
		...
		Mat x1(2, n), x2(2, n);
		size_t cptIndex = 0;
		for (const std::pair< size_t, submapTrack > &track : map_tracksCommon)
		{
			//Get corresponding point
			tracks::submapTrack::const_iterator iter = track.second.begin();
		...
		{
			tracks::submapTrack::const_iterator iter = track.second.begin();
			tracks::submapTrack::const_iterator iter2 = track.second.begin();
			std::advance(iter2, 1);
	...
	bool IncrementalReconstructionEngine::FindImagesWithPossibleResection(std::vector<size_t> & possibleIndexes)
	{
		...
		std::vector< std::pair<size_t, size_t> > putative; // ImageId, NbPutativeCommonPoint
		for (const auto& index : _set_remainingImageId)
		{
			// Compute 2D - 3D possible content
			openMVG::tracks::STLMAPTracks tracksCommon;
			std::set<size_t> imageIndex;
			imageIndex.insert(index);
			TracksUtilsMap::GetTracksInImages(imageIndex, _map_tracks, tracksCommon);

			if (!tracksCommon.empty())
			{
				std::set<size_t> set_tracksIds;
				TracksUtilsMap::GetTracksIdVector(tracksCommon, &set_tracksIds);
				...
				putative.push_back(make_pair(index, vec_trackIdForResection.size()));
			}
		}
		...
		{
			sort(putative.begin(), putative.end(), sort_pair_second<size_t, size_t, std::greater<size_t> >());

			size_t M = putative[0].second;
			possibleIndexes.push_back(putative[0].first);
			...
			for (size_t i = 1; i < putative.size() && putative[i].second > threshold; ++i)
			{
				possibleIndexes.push_back(putative[i].first);
			}
			return true;
		}
	}

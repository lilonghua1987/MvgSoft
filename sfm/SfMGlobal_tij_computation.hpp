  ...
  tracks::mapPairWiseMatches & newpairMatches)
{
  ...
  int cpuN = (std::min)((int)std::thread::hardware_concurrency(), (int)vec_triplets.size());
  if (cpuN >= 1)
  {
	  std::cout << "runTripletThread -> CPU Cores :" << cpuN << std::endl;
	  std::vector<std::thread> threads(cpuN);

	  for (auto& t : threads) t = std::thread([&vec_triplets, &map_tracksPerTriplets](GlobalReconstructionEngine& arg){ arg.runTripletThread(vec_triplets, map_tracksPerTriplets); }, std::ref(*this));

	  for (auto& t : threads) t.join();

	  std::cout << "map_tracksPerTriplets size = " << map_tracksPerTriplets.size() << std::endl;
  }
  ...
  cpuN = (std::min)((int)std::thread::hardware_concurrency(), (int)vec_edges.size());
  if (cpuN >= 1)
  {
	  std::cout << "Computation of the relative translations -> CPU Cores :" << cpuN << std::endl;
	  std::vector<std::thread> threads(cpuN);
	  ...
	  for (auto& t : threads) t = std::thread([&](){ 
		  ...
		  {
			  int && k = jobs++;
			  if (k >= static_cast<int>(vec_edges.size())) break;

			  //std::cout << "Computation relative translations ThreadID = " << std::this_thread::get_id() << ", jobs = " << k << std::endl;
...
void GlobalReconstructionEngine::runTripletThread(const std::vector< graphUtils::Triplet > & triplets, std::map<size_t, size_t>& mapTracksPerTriplets)
{
	while (true)
	{
		int && i = jobs++;

		if (i >= static_cast<int>(triplets.size())) break;

		const graphUtils::Triplet & triplet = triplets[i];
		...
		std::unique_lock<std::mutex> lck(mtx);
		mapTracksPerTriplets[i] = map_tracks.size();
	}
}

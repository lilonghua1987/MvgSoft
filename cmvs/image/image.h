...
class Cimage {
 public:
  ...
  void sift(const pmvs::Vec3f& center, const pmvs::Vec3f& xaxis, const pmvs::Vec3f& yaxis,
  ...
  inline pmvs::Vec3f getColor(const int ix, const int iy, const int level) const;

  inline void setColor(const int ix, const int iy, const int level,
                       const pmvs::Vec3f& rgb);
  ...
  inline int isSafe(const pmvs::Vec3f& icoord, const int level) const;
  ...
  static void rgb2hsv(const pmvs::Vec3f& rgb, pmvs::Vec3f& hsv);
  static void rgb2hsv(const pmvs::Vec3f& rgb, float& hr, float& sr, float& vr);
  static void rgb2hsv(const float r, const float g, const float b, pmvs::Vec3f& hsv);  
  
  static void rgb2hs(const float r, const float g, const float b,
		     float& h, float& s);
  static void rgb2hs(const pmvs::Vec3f& rgb, float& h, float& s);
  static void rgb2hs(const pmvs::Vec3f& rgb, pmvs::Vec2f& hs);
  static void rgb2hs(const float r, const float g, const float b, pmvs::Vec2f& hs);
...
inline int Cimage::isSafe(const pmvs::Vec3f& icoord, const int level) const {
...
pmvs::Vec3f Cimage::getColor(const float x, const float y, const int level) const{
  ...
  return pmvs::Vec3f(r, g, b);
  /*
  return pmvs::Vec3f(m_dimages[level][index] * f00 + m_dimages[level][index + 3] * f10 +
  ...
  */
  ...
  return pmvs::Vec3f(r, g, b);
  /*
  return pmvs::Vec3f(m_images[level][index] * f00 + m_images[level][index + 3] * f10 +
  ...
  */
  ...
  const pmvs::Vec3f vc[2][2] = {{getColor(lx, ly, level), getColor(lx, uy, level)},
			  {getColor(ux, ly, level), getColor(ux, uy, level)}};
  
  const pmvs::Vec3f color = vc[0][0] * (ux - x) * (uy - y) + vc[0][1] * (ux - x) * (y - ly) +
...
pmvs::Vec3f Cimage::getColor(const int ix, const int iy, const int level) const{
#ifdef FURUKAWA_DEBUG
  if (m_alloc != 2) {
    std::cerr << "First allocate" << std::endl;
    exit (1);
...
#ifdef FURUKAWA_IMAGE_GAMMA
  return pmvs::Vec3f(m_dimages[level][index],
	       m_dimages[level][index+1],
	       m_dimages[level][index+2]);
#else
  return pmvs::Vec3f(m_images[level][index],

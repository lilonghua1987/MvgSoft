...
#define max3(a, b, c) ((a)>(b) ? ((a)>(c) ? (a) : (c)) : ((b)>(c) ? (b) : (c)))
	...
	void Cimage::completeName(const std::string& lhs, std::string& rhs,
		...
		// ppm jpg
		if (color) {
			std::string stmp0 = lhs + ".ppm";    std::string stmp1 = lhs + ".jpg";
		...
		// pgm pbm
			...
			std::string stmp0 = lhs + ".pgm";    std::string stmp1 = lhs + ".pbm";
	...
	void Cimage::alloc(const int fast, const int filter) {
		...
		if (m_name.length() < 3) {
			std::cerr << "Image file name has less than 3 characters." << std::endl
				<< "Cannot allocate: " << m_name << std::endl;
			exit(1);
		}
		...
		if (readJpegImage(m_name, m_images[0], m_widths[0], m_heights[0], fast) == 0 &&
			...
			std::cerr << "Only jpeg and ppm formats are allowed. Stop allocation: "
				<< m_name << std::endl;
			return;
		}
		...
		if (!m_mname.empty()) {
			if (readPGMImage(m_mname, m_masks[0], m_widths[0], m_heights[0], 0) ||
				...
				// 255: in, 0 : out
				std::cerr << "Read mask: " << m_mname << std::endl;
		...
		if (!m_ename.empty()) {
			if (readPGMImage(m_ename, m_edges[0], m_widths[0], m_heights[0], 0) ||
				...
				std::cerr << "Read edge: " << m_ename << std::endl;
				// 255: in, 0 : out
	...
	*/
...
#else
			std::vector<unsigned char>().swap(m_images[l]);
#endif
			if (!m_masks.empty())
				std::vector<unsigned char>().swap(m_masks[l]);
			if (!m_edges.empty())
				std::vector<unsigned char>().swap(m_edges[l]);
		}
	}
	...
	void Cimage::free(void) {
		...
		std::vector<std::vector<unsigned char> >().swap(m_images);
		std::vector<std::vector<unsigned char> >().swap(m_masks);
		std::vector<std::vector<unsigned char> >().swap(m_edges);
	...
	void Cimage::buildImage(const int filter) {
		pmvs::Mat4 mask;
		mask[0] = pmvs::Vec4(1.0, 3.0, 3.0, 1.0);  mask[1] = pmvs::Vec4(3.0, 9.0, 9.0, 3.0);
		mask[2] = pmvs::Vec4(3.0, 9.0, 9.0, 3.0);  mask[3] = pmvs::Vec4(1.0, 3.0, 3.0, 1.0);
					...
					for (int j = -1; j < 3; ++j) {
						...
						for (int i = -1; i < 3; ++i) {
							...
							if (filter == 0) {
								...
								color[0] = std::max(color[0], (double)m_images[level - 1][index]);
								color[1] = std::max(color[1], (double)m_images[level - 1][index + 1]);
								color[2] = std::max(color[2], (double)m_images[level - 1][index + 2]);
							}
							else {
								color[0] = std::min(color[0], (double)m_images[level - 1][index]);
								color[1] = std::min(color[1], (double)m_images[level - 1][index + 1]);
								color[2] = std::min(color[2], (double)m_images[level - 1][index + 2]);
							}
#endif
						}
					}
	...
	void Cimage::buildMask(void) {
		...
		for (int level = 1; level < m_maxLevel; ++level) {
			...
			for (int y = 0; y < m_heights[level]; ++y) {
				const int ys[2] = { 2 * y, std::min(m_heights[level - 1] - 1, 2 * y + 1) };
				for (int x = 0; x < m_widths[level]; ++x) {
					const int xs[2] = { 2 * x, std::min(m_widths[level - 1] - 1, 2 * x + 1) };
					int in = 0;	int out = 0;
	...
	int Cimage::readPBMImage(const std::string file,
		...
		std::string header;  unsigned char uctmp;
	...
	int Cimage::readPGMImage(const std::string file,
		...
		std::string header;  unsigned char uctmp;  int itmp;
	...
	int Cimage::readPPMImage(const std::string file,
		...
		ifstr.read((char*)&uctmp, sizeof(unsigned char));
		if (header != "P6") {
			std::cerr << "Only accept binary ppm format: " << file << std::endl;
			return 0;
		}
	...
	int Cimage::readJpegImage(const std::string file,
		...
		*/
		...
		{
			std::cerr << "Cannot handle this component. Component num is " << component << std::endl;
			exit(1);
		}
		return 1;
	}
	...
	void Cimage::rgb2hs(const pmvs::Vec3f& rgb, float& h, float& s) {
		rgb2hs(rgb[0], rgb[1], rgb[2], h, s);
	}

	void Cimage::rgb2hs(const pmvs::Vec3f& rgb, pmvs::Vec2f& hs) {
		rgb2hs(rgb[0], rgb[1], rgb[2], hs[0], hs[1]);
	}

	void Cimage::rgb2hs(const float r, const float g, const float b, pmvs::Vec2f& hs) {
	...
	void Cimage::rgb2hsv(const pmvs::Vec3f& rgb, pmvs::Vec3f& hsv) {
		rgb2hsv(rgb[0], rgb[1], rgb[2], hsv[0], hsv[1], hsv[2]);
	}

	void Cimage::rgb2hsv(const pmvs::Vec3f& rgb, float& hr, float& sr, float& vr) {
		rgb2hsv(rgb[0], rgb[1], rgb[2], hr, sr, vr);
	}

	void Cimage::rgb2hsv(const float r, const float g, const float b, pmvs::Vec3f& hsv) {
	...
	float Cimage::hsdis(const float h0, const float s0, const float h1, const float s1) {
		...
		pmvs::Vec2f axis0(cos(angle0), sin(angle0));
		axis0 *= s0;

		const float angle1 = h1 * M_PI / 180.0;
		pmvs::Vec2f axis1(cos(angle1), sin(angle1));
		axis1 *= s1;
	...
	void Cimage::sift(const pmvs::Vec3f& center,
		const pmvs::Vec3f& xaxis, const pmvs::Vec3f& yaxis,
		std::vector<float>& descriptor) const {
		const float step = (norm(xaxis) + norm(yaxis)) / 2.0f;
		const int level = std::max(0, std::min(m_maxLevel - 1,
	...
	void Cimage::sift(const pmvs::Vec3f& center,
		...
		*/
		...
		const pmvs::Vec3f topleft = center - width205 * yaxis - width205 * xaxis;
		const pmvs::Vec3f topright = center - width205 * yaxis + width205 * xaxis;
		const pmvs::Vec3f bottomleft = center + width205 * yaxis - width205 * xaxis;
		const pmvs::Vec3f bottomright = center + width205 * yaxis + width205 * xaxis;

		const float minx = std::min(std::min(topleft[0], topright[0]),
			std::min(bottomleft[0], bottomright[0]));
		const float maxx = std::max(std::max(topleft[0], topright[0]),
			std::max(bottomleft[0], bottomright[0]));
		const float miny = std::min(std::min(topleft[1], topright[1]),
			std::min(bottomleft[1], bottomright[1]));
		const float maxy = std::max(std::max(topleft[1], topright[1]),
			std::max(bottomleft[1], bottomright[1]));
		...
		for (int y = 0; y < width; ++y) {
			pmvs::Vec3f start = topleft + y * yaxis;
			...
			for (int x = 0; x < width; ++x) {
				...
				const pmvs::Vec3f px = start - xaxis;   const pmvs::Vec3f nx = start + xaxis;
				const pmvs::Vec3f py = start - yaxis;   const pmvs::Vec3f ny = start + yaxis;

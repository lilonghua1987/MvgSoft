...
namespace cpmvs_image
{
	...
	float Cphoto::ssd(const std::vector<pmvs::Vec3f>& tex0,
	...
	float Cphoto::idot(const std::vector<pmvs::Vec3f>& tex0,
		const std::vector<pmvs::Vec3f>& tex1) {
	...
	void Cphoto::idotC(const std::vector<pmvs::Vec3f>& tex0,
	...
	void Cphoto::normalize(std::vector<pmvs::Vec3f>& tex) {
		//----------------------------------------------------------------------
		// normalize average
		pmvs::Vec3f ave;
	...
	void Cphoto::grabTex(const int level, const pmvs::Vec2f& icoord,
		const pmvs::Vec2f& xaxis, const pmvs::Vec2f& yaxis,
		const int size, std::vector<pmvs::Vec3f>& tex,
		...
		for (int y = -margin; y <= margin; ++y) {
			pmvs::Vec2f v2ftmp = icoord - margin * xaxis + y * yaxis;
	...
	void Cphoto::grabTex(const int level, const pmvs::Vec4f& coord,
		...
		const pmvs::Vec3f icoord3 = project(coord, level);
		const pmvs::Vec2f icoord(icoord3[0], icoord3[1]);

		const pmvs::Vec3f xaxis3 = project(coord + pxaxis * scale, level) - icoord3;
		const pmvs::Vec2f xaxis(xaxis3[0], xaxis3[1]);

		const pmvs::Vec3f yaxis3 = project(coord + pyaxis * scale, level) - icoord3;
		const pmvs::Vec2f yaxis(yaxis3[0], yaxis3[1]);

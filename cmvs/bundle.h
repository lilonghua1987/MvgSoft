...
class Cbundle {
 public:
  ...
  bool run(const std::string prefix, const int imageThreshold,
  ...
  // Point params
  std::vector<pmvs::Vec4f> m_coords;
  std::vector<std::vector<int> > m_visibles;

  std::vector<pmvs::Vec3f> m_colors;
 ...
 protected:
  bool  prep(const std::string prefix, const int imageThreshold,
            const int tau, const float scoreRatioThreshold,
            const float coverageThreshold, const int pnumThreshold,
            const int CPU);
  ...
  bool readBundle(const std::string file);
  ...
  // angle score
  static float angleScore(const pmvs::Vec4f& ray0, const pmvs::Vec4f& ray1);
  ...
  float computeScore2(const pmvs::Vec4f& coord,

...
namespace mvg
{
	ComputeMatchH::ComputeMatchH(QObject *parent)
		: ComputeMatch(parent, "log/ComputeMatchH.log")
	{
		model = hMatrix;
		modelFileName = "matches.h.txt";
	}

	ComputeMatchH::~ComputeMatchH()
	{

	}

	void ComputeMatchH::run()
	{
		//---------------------------------------
		// a. List images
		//---------------------------------------
	...
	void ComputeMatchH::extractFeature()
	{
		...
		for (auto& t : threads) t = std::thread([](ComputeMatchH& arg){arg.runFThread(); }, std::ref(*this));

		for (auto& t : threads) t.join();
	}

	void ComputeMatchH::putativeMatch()
	{
		...
		else // Compute the putatives matches
		{
			...
			if (!mapFeat.empty() && !mapDesc.empty())
			{
				...
				for (auto& t : threads) t = std::thread([](ComputeMatchH& arg){arg.runMThread(); }, std::ref(*this));
	...
	void ComputeMatchH::postProcess()
	{
		if (!mapFeat.empty() && !mapDesc.empty())
		{
			...
			filter = std::unique_ptr<GeometricFilter_HMatrix_AC>(new GeometricFilter_HMatrix_AC(maxResidualError));
			std::vector<std::thread> threads(cpuN);

			for (auto& t : threads) t = std::thread([](ComputeMatchH& arg){arg.runGThread(); }, std::ref(*this));

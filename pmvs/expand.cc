...
void Cexpand::run(void)
{
  ...
  if (cpuN < 1) return;
  std::cout << "Cexpand -> CPU Cores :" << cpuN << std::endl;
  std::vector<std::thread> threads(cpuN);

  for (auto& t : threads) t = std::thread([](Cexpand& arg){arg.expandThread(); }, std::ref(*this));
...
void Cexpand::expandThread(void)
{
  ...
  while (true) 
  {
    ...
    // For each direction;
    vector<vector<pmvs::Vec4f> > canCoords;
...
void Cexpand::findEmptyBlocks(const Ppatch& ppatch,
			      std::vector<std::vector<pmvs::Vec4f> >& canCoords) {
  ...
  while (bpatch != epatch) {
    const pmvs::Vec4f diff = (*bpatch)->m_coord - ppatch->m_coord;
    pmvs::Vec2f f2(diff * xdir, diff * ydir);
  ...
  for (int i = 0; i < dnum; ++i) {
    ...
    pmvs::Vec4f canCoord = ppatch->m_coord +
...
int Cexpand::checkCounts(Patch::Cpatch& patch) {
  ...
  vector<pmvs::Vec2i>::iterator begin2 = patch.m_grids.begin();
...
int Cexpand::updateCounts(const Cpatch& patch) {
...

  {
	...
	vector<pmvs::Vec2i>::const_iterator begin2 = patch.m_grids.begin();
...

  {
	...
	vector<pmvs::Vec2i>::const_iterator begin2 = patch.m_vgrids.begin();

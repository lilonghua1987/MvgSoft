...
void CpatchOrganizerS::writePatches2(const std::string prefix) {
	collectPatches(1);
	{
		...
		std::cout << "PLY :" << prefix + ".ply" << std::endl;
		writePLY(m_ppatches, prefix + ".ply");
	}

	{
		ofstream ofstr(prefix + ".patch");
		ofstr << "PATCHES" << endl
...
void CpatchOrganizerS::addPatch(Patch::Ppatch& ppatch) {
	...
	vector<pmvs::Vec2i>::iterator bgrid = ppatch->m_grids.begin();
	while (bimage != eimage) {
...
void CpatchOrganizerS::updateDepthMaps(Ppatch& ppatch) {
	for (int image = 0; image < m_fm.m_tnum; ++image) {
		const pmvs::Vec3f icoord = m_fm.m_pss.project(image, ppatch->m_coord, m_fm.m_level);
	...
	const std::vector<int>& images) const
{
	...
	while (bimage != eimage) 
	{
			...
			0 <= iy && iy < m_gheights[*bimage]) 
		{
			...
			patch.m_grids.push_back(pmvs::Vec2i(ix, iy));
		}
		++bimage;
	}
}
...
void CpatchOrganizerS::setGrids(Cpatch& patch) const
{
	...
	for (int i = 0; i < (int)patch.m_images.size(); ++i) 
	{
		...
		pmvs::Vec3f icoord = m_fm.m_pss.project(image, patch.m_coord, m_fm.m_level);
		const int ix = ((int)floor(icoord[0] + 0.5f)) / m_fm.m_csize;
		const int iy = ((int)floor(icoord[1] + 0.5f)) / m_fm.m_csize;
		patch.m_grids.push_back(pmvs::TVec2<int>(ix, iy));
	}
}
...
void CpatchOrganizerS::setVImagesVGrids(Cpatch& patch) 
{
	...
	for (int image = 0; image < m_fm.m_tnum; ++image) 
	{
		...
		patch.m_vgrids.push_back(pmvs::TVec2<int>(ix, iy));
	}
}
...
int CpatchOrganizerS::isVisible(const Cpatch& patch, const int image,
	...
	pmvs::Vec4f ray = patch.m_coord - m_fm.m_pss.m_photos[image].m_center;
	unitize(ray);
...
void CpatchOrganizerS::findNeighbors(const Patch::Cpatch& patch,
	...
	vector<pmvs::Vec2i>::const_iterator bgrid = patch.m_grids.begin();
...
void CpatchOrganizerS::setScales(Patch::Cpatch& patch) const {
	...
	pmvs::Vec4f ray = patch.m_coord - m_fm.m_pss.m_photos[patch.m_images[0]].m_center;
	unitize(ray);
	...
	for (int i = 1; i < inum; ++i) {
		pmvs::Vec3f diff = m_fm.m_pss.project(patch.m_images[i], patch.m_coord, m_fm.m_level) -
...
void CpatchOrganizerS::writePLY(const std::vector<Ppatch>& patches,
	...
	while (bpatch != bend) {
		...
		if (mode == 0) {
			...
			while (bimage != eimage) {
				const int index = *bimage;
				pmvs::Vec4f ray = m_fm.m_pss.m_photos[index].m_center - (*bpatch)->m_coord;
				ray[3] = 0.0f;
				unitize(ray);
			...
			float r, g, b;
			cpmvs_image::Cimage::gray2rgb(angle, r, g, b);
...
void CpatchOrganizerS::writePLY(const std::vector<Ppatch>& patches,
	const std::string filename,
	const std::vector<pmvs::Vec3i>& colors) {
	ofstream ofstr;
	ofstr.open(filename.c_str());
	ofstr << "ply" << endl
		<< "format ascii 1.0" << endl
	...
	vector<Ppatch>::const_iterator bpatch = patches.begin();
	vector<Ppatch>::const_iterator bend = patches.end();
	vector<pmvs::Vec3i>::const_iterator colorb = colors.begin();

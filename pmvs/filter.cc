...
void Cfilter::filterOutside(void)
{
	...
	int cpuN = (int)std::thread::hardware_concurrency();
	if (cpuN < 1) return;
	std::cout << "Cfilter -> CPU Cores :" << cpuN << std::endl;
	std::vector<std::thread> threads(cpuN);

	for (auto& t : threads) t = std::thread([](Cfilter& arg){arg.filterOutsideThread(); }, std::ref(*this));
...
float Cfilter::computeGain(const Patch::Cpatch& patch, const int lock) {
	...
	for (int i = 0; i < size; ++i) {
		...
		if (lock)
		{
			std::unique_lock<std::mutex> lck(m_fm.mtx);
...
void Cfilter::filterExact(void) {
	...
	int cpuN = (int)std::thread::hardware_concurrency();
	if (cpuN < 1) return;
	std::cout << "filterExact -> CPU Cores :" << cpuN << std::endl;
	std::vector<std::thread> threads(cpuN);

	for (auto& t : threads) t = std::thread([](Cfilter& arg){arg.filterExactThread(); }, std::ref(*this));
...
void Cfilter::filterExactThread(void) {
	...
	vector<vector<pmvs::TVec2<int> > > newgrids, removegrids;
			...
			for (int x = 0; x < w; ++x) {
				...
				for (int i = 0; i < (int)m_fm.m_pos.m_pgrids[image][index].size(); ++i) {
					...
					if (safe) {
						...
						newgrids[patch.m_id].push_back(pmvs::TVec2<int>(x, y));
					}
					else {
						removeimages[patch.m_id].push_back(image);
						removegrids[patch.m_id].push_back(pmvs::TVec2<int>(x, y));
					}
				}
			}
		}
...
int Cfilter::filterQuad(const Patch::Cpatch& patch,
	...
	for (int n = 0; n < nsize; ++n) {
		A[n].resize(5);
		pmvs::Vec4f diff = neighbors[n]->m_coord - patch.m_coord;
		fxs[n] = diff * xdir / h;
		fys[n] = diff * ydir / h;
...
void Cfilter::filterNeighbor(const int times) 
{
	...
	for (m_time = 0; m_time < times; ++m_time)
	{
		...
		int cpuN = (int)std::thread::hardware_concurrency();
		if (cpuN < 1) return;
		std::cout << "filterNeighbor -> CPU Cores :" << cpuN << std::endl;
		std::vector<std::thread> threads(cpuN);

		for (auto& t : threads) t = std::thread([](Cfilter& arg){arg.filterNeighborThread(); }, std::ref(*this));
...
void Cfilter::setDepthMaps(void) 
{
	...
	int cpuN = (int)std::thread::hardware_concurrency();
	if (cpuN < 1) return;
	std::cout << "setDepthMaps -> CPU Cores :" << cpuN << std::endl;
	std::vector<std::thread> threads(cpuN);

	for (auto& t : threads) t = std::thread([](Cfilter& arg){arg.setDepthMapsThread(); }, std::ref(*this));
...
void Cfilter::setDepthMapsVGridsVPGridsAddPatchV(const int additive) 
{
	...
	int cpuN = (int)std::thread::hardware_concurrency();
	if (cpuN < 1) return;
	std::cout << "setVGridsVPGridsThread -> CPU Cores :" << cpuN << std::endl;
	std::vector<std::thread> threads0(cpuN);

	for (auto& t : threads0) t = std::thread([](Cfilter& arg){arg.setVGridsVPGridsThread(); }, std::ref(*this));

	for (auto& t : threads0) t.join();

	m_fm.m_count = 0;

	std::cout << "addPatchVThread -> CPU Cores :" << cpuN << std::endl;
	std::vector<std::thread> threads1(cpuN);

	for (auto& t : threads1) t = std::thread([](Cfilter& arg){arg.addPatchVThread(); }, std::ref(*this));
...
void Cfilter::addPatchVThread(void)
{
	while (true) 
	{
		...
		while (bpatch != epatch) 
		{
			...
			vector<pmvs::Vec2i>::iterator bgrid = ppatch->m_vgrids.begin();
			while (bimage != eimage)
			{
				if (*bimage == index) 
				{

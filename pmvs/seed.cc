...
void Cseed::run(void)
{
	...
	if (cpuN < 1) return;
	std::cout << "Cseed -> CPU Cores :" << cpuN << std::endl;
	std::vector<std::thread> threads(cpuN);

	for (auto& t : threads) t = std::thread([](Cseed& arg){arg.initialMatchThread(); }, std::ref(*this));
...
void Cseed::collectCells(const int index0, const int index1,
	const Cpoint& p0, std::vector<pmvs::Vec2i>& cells) {
	pmvs::Vec3 point(p0.m_icoord[0], p0.m_icoord[1], p0.m_icoord[2]);
#ifdef DEBUG
	...
	pmvs::Mat3 F;
	cpmvs_image::setF(m_fm.m_pss.m_photos[index0], m_fm.m_pss.m_photos[index1],
		F, m_fm.m_level);
	const int gwidth = m_fm.m_pos.m_gwidths[index1];
	const int gheight = m_fm.m_pos.m_gheights[index1];

	pmvs::Vec3 line = transpose(F) * point;
	...
	if (fabs(line[0]) > fabs(line[1])) {
		for (int y = 0; y < gheight; ++y) {
			...
			if (0 <= ix && ix < gwidth)
				cells.push_back(pmvs::TVec2<int>(ix, y));
			if (0 <= ix - 1 && ix - 1 < gwidth)
				cells.push_back(pmvs::TVec2<int>(ix - 1, y));
			if (0 <= ix + 1 && ix + 1 < gwidth)
				cells.push_back(pmvs::TVec2<int>(ix + 1, y));
		}
	}
		...
		for (int x = 0; x < gwidth; ++x) {
			...
			if (0 <= iy && iy < gheight)
				cells.push_back(pmvs::TVec2<int>(x, iy));
			if (0 <= iy - 1 && iy - 1 < gheight)
				cells.push_back(pmvs::TVec2<int>(x, iy - 1));
			if (0 <= iy + 1 && iy + 1 < gheight)
				cells.push_back(pmvs::TVec2<int>(x, iy + 1));
		}
	}
...
void Cseed::collectCandidates(const int index, const std::vector<int>& indexes,
	...
	const pmvs::Vec3 p0(point.m_icoord[0], point.m_icoord[1], 1.0);
	for (int i = 0; i < (int)indexes.size(); ++i) {
		const int indexid = indexes[i];

		vector<pmvs::TVec2<int> > cells;
		collectCells(index, indexid, point, cells);
		pmvs::Mat3 F;
		cpmvs_image::setF(m_fm.m_pss.m_photos[index], m_fm.m_pss.m_photos[indexid],
			F, m_fm.m_level);
		...
		for (int i = 0; i < (int)cells.size(); ++i) {
			...
			while (begin != end) {
				...
				const pmvs::Vec3 p1(rhs.m_icoord[0], rhs.m_icoord[1], 1.0);
				if (m_fm.m_epThreshold <= cpmvs_image::computeEPD(F, p0, p1)) {
					++begin;
					continue;
				}
				vcp.push_back(*begin);

...
class Coptim {
 public:
  ...
  float getUnit(const int index, const pmvs::Vec4f& coord) const;
 ...
 protected:
  ...
  int grabTex(const pmvs::Vec4f& coord, const pmvs::Vec4f& pxaxis, const pmvs::Vec4f& pyaxis,
              const pmvs::Vec4f& pzaxis, const int index, const int size,
              std::vector<float>& tex) const;
  
  int grabSafe(const int index, const int size, const pmvs::Vec3f& center,
               const pmvs::Vec3f& dx, const pmvs::Vec3f& dy, const int level) const;

  double computeSSD(const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
  ...
  /*
  double computeINCC(const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
                     const std::vector<int>& indexes, const int id,
                     const int robust);
  */
  double computeINCC(const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
 ...
 protected:
  ...
  void encode(const pmvs::Vec4f& coord,
              double* const vect, const int id) const;
  void encode(const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
              double* const vect, const int id) const;
  void decode(pmvs::Vec4f& coord, pmvs::Vec4f& normal,
	      const double* const vect, const int id) const;
  void decode(pmvs::Vec4f& coord, const double* const vect, const int id) const;
 ...
 protected:
  ...
  //-----------------------------------------------------------------
  // Axes
  std::vector<pmvs::Vec3f> m_xaxes;
  std::vector<pmvs::Vec3f> m_yaxes;
  std::vector<pmvs::Vec3f> m_zaxes;
  // Scales
  ...
  std::vector<pmvs::Vec4f> m_centersT;
  std::vector<pmvs::Vec4f> m_raysT;
  ...
  std::vector<pmvs::Vec3f> m_paramsT;

...
void Coptim::setAxesScales(void) {
  ...
  for (int index = 0; index < m_fm.m_num; ++index) {
    m_zaxes[index] = pmvs::Vec3f(m_fm.m_pss.m_photos[index].m_oaxis[0],
                           m_fm.m_pss.m_photos[index].m_oaxis[1],
                           m_fm.m_pss.m_photos[index].m_oaxis[2]);
    m_xaxes[index] = pmvs::Vec3f(m_fm.m_pss.m_photos[index].m_projection[0][0][0],
  ...
  for (int index = 0; index < m_fm.m_num; ++index) {
    const pmvs::Vec4f xaxe(m_xaxes[index][0], m_xaxes[index][1], m_xaxes[index][2], 0.0);
    const pmvs::Vec4f yaxe(m_yaxes[index][0], m_yaxes[index][1], m_yaxes[index][2], 0.0);
...
void Coptim::collectImages(const int index, std::vector<int>& indexes) const{
  ...
  indexes.clear();
  pmvs::Vec4f ray0 = m_fm.m_pss.m_photos[index].m_oaxis;
  ray0[3] = 0.0f;

  vector<pmvs::Vec2f> candidates;
  ...
  for (int i = 0; i < (int)m_fm.m_visdata2[index].size(); ++i) {
    ...
    pmvs::Vec4f ray1 = m_fm.m_pss.m_photos[indextmp].m_oaxis;
    ray1[3] = 0.0f;

    if (ray0 * ray1 < cos(m_fm.m_angleThreshold0))
      continue;
    
	candidates.push_back(pmvs::Vec2f(m_fm.m_pss.m_distances[index][indextmp], indextmp));
  }
  
  sort(candidates.begin(), candidates.end(), pmvs::Svec2cmp<float>());
...
void Coptim::filterImagesByAngle(Cpatch& patch) {
  ...
  while (bimage != eimage) {
    ...
    pmvs::Vec4f ray = m_fm.m_pss.m_photos[index].m_center - patch.m_coord;
    unitize(ray);
...
void Coptim::addImages(Patch::Cpatch& patch) const{
  ...
  while (bimage != eimage) {
    ...
    const pmvs::Vec3f icoord = m_fm.m_pss.project(*bimage, patch.m_coord, m_fm.m_level);
    ...
    pmvs::Vec4f ray = m_fm.m_pss.m_photos[*bimage].m_center - patch.m_coord;
    unitize(ray);
...
void Coptim::computeUnits(const Patch::Cpatch& patch,
                          ...
                          std::vector<pmvs::Vec4f>& rays) const{
  vector<int>::const_iterator bimage = patch.m_images.begin();
  vector<int>::const_iterator eimage = patch.m_images.end();
...
void Coptim::encode(const pmvs::Vec4f& coord,
		    double* const vect, const int id) const {
  vect[0] = (coord - m_centersT[id]) * m_raysT[id] / m_dscalesT[id];
}

void Coptim::encode(const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
		    double* const vect, const int id) const {
...
void Coptim::decode(pmvs::Vec4f& coord, pmvs::Vec4f& normal,
  ...
  pmvs::Vec3f ftmp = m_xaxes[image] * fx + m_yaxes[image] * fy + m_zaxes[image] * fz;
  normal = pmvs::Vec4f(ftmp[0], ftmp[1], ftmp[2], 0.0f);
}

void Coptim::decode(pmvs::Vec4f& coord, const double* const vect, const int id) const {
...
int Coptim::grabSafe(const int index, const int size, const pmvs::Vec3f& center,
                     const pmvs::Vec3f& dx, const pmvs::Vec3f& dy, const int level) const {
  const int margin = size / 2;

  const pmvs::Vec3f tl = center - dx * margin - dy * margin;
  const pmvs::Vec3f tr = center + dx * margin - dy * margin;

  const pmvs::Vec3f bl = center - dx * margin + dy * margin;
  const pmvs::Vec3f br = center + dx * margin + dy * margin;
...
int Coptim::grabTex(const pmvs::Vec4f& coord, const pmvs::Vec4f& pxaxis, const pmvs::Vec4f& pyaxis,
		    const pmvs::Vec4f& pzaxis, const int index, const int size,
                    std::vector<float>& tex) const {
  tex.clear();

  pmvs::Vec4f ray = m_fm.m_pss.m_photos[index].m_center - coord;
  unitize(ray);
  ...
  pmvs::Vec3f center = m_fm.m_pss.project(index, coord, m_fm.m_level);  
  pmvs::Vec3f dx = m_fm.m_pss.project(index, coord + pxaxis, m_fm.m_level) - center;
  pmvs::Vec3f dy = m_fm.m_pss.project(index, coord + pyaxis, m_fm.m_level) - center;
  ...
  pmvs::Vec3f left = center - dx * margin - dy * margin;

  tex.resize(3 * size * size);
  float* texp = &tex[0] - 1;
  for (int y = 0; y < size; ++y) {
    pmvs::Vec3f vftmp = left;
    left += dy;
    for (int x = 0; x < size; ++x) {
      pmvs::Vec3f color = m_fm.m_pss.getColor(index, vftmp[0], vftmp[1], newlevel);
      *(++texp) = color[0];
      *(++texp) = color[1];
      *(++texp) = color[2];
      vftmp += dx;
...
double Coptim::computeINCC(const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
...
double Coptim::computeINCC(const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
			   const std::vector<int>& indexes, const pmvs::Vec4f& pxaxis,
			   const pmvs::Vec4f& pyaxis, const int id,
                           const int robust) {
  if ((int)indexes.size() < 2)
    return 2.0;
...
float Coptim::getUnit(const int index, const pmvs::Vec4f& coord) const {
...
void Coptim::getPAxes(const int index, const pmvs::Vec4f& coord, const pmvs::Vec4f& normal,
  ...
  pmvs::Vec3f yaxis3 = cross(normal3, m_xaxes[index]);
  unitize(yaxis3);
